...
Now, what to do about the exiting from idle time. This way seems to run things twice. since we save in agave, the first change after it makes this run straight away. Maybe a heuristic approach, e.g. track time and only run this if it is some interval past the last time.
